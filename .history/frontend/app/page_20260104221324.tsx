...
export default function Home() {
  ...
  const handleSolve = async () => {
    ...
    try {
      const response = await fetch('https://ssm03-connections-solver-ai.hf.space/solve', {
        method: 'POST',
        headers: {
          ...
          {results && results.length > 0 && (
            <div className="solutions-list">
              {results.slice(0, 4).map((result) => {
                const categoryColor = getCategoryColor(result.rank)
                const probability = result.score * 100
                return (
                  <div key={result.rank} className="solution-item">
                    <div className="solution-header">
                      <div className="rank-square" style={{ backgroundColor: categoryColor }}>
                        {result.rank}
                      </div>
                      <div className="solution-info">
                        <span className="category-name">{getCategoryName(result.rank)}</span>
                        <span className="probability-text">Probability: {probability.toFixed(2)}%</span>
                      </div>
                    </div>
                    ...
                    {result.groups.length > 0 && (
                      <div className="solution-words">
                        {result.groups[0].join(', ').toUpperCase()}
                      </div>
                    )}
                  </div>
                )
